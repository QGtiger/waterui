import { addClass, addHandler, AniFactory, promiseAwaitFunc } from '@lightfish/tools';
  ...
  return function (ModalNode: ReactComponent) {
    ...
    > {
      ...
      constructor(props: any) {
        ...
        if (isReactClassComponent(ModalNode) && this.props.closeModal) {
          ModalNode.prototype.closeModal = this.props.closeModal;
        }
      }
...
export class WTIModal {
   ...
   */
  forceUpdate() {
    if (this.modalIns && this.state === ModalState.SHOWED) {
  ...
  @PromiseAwait<ModalState>(
    'state',
    [ModalState.BEFORESHOW, ModalState.SHOWED],
    ...
    return new Promise<void>(async (resolve) => {
      ...
      if (this.modalIns) {
        ...
        try {
          ...
          const preloadPromise = wrapPromise(
            onPreloadResource(
              this.modalCfg.preloadResource,
              this.modalCfg.parallelMode,
              this.modalCfg.preloadResourceFunc,
            ),
          );
          const FtComp = makeObserverComp(() => {
            ...
            return (
              <Component
                ...
                closeModal={this.forceClose.bind(this)}
                {...this.props}
              />
            );
          });
